...
moriade.menu.keytable = ui.new_label("AA", "Fake lag", "\aAFAFC3FF•------------------------------------------------•");
...
moriade.menu.breaklc = ui.new_checkbox("AA", "Fake lag", "\aFFFFFFFFAuto teleport")
moriade.menu.breaklckey = ui.new_hotkey("AA", "Fake lag", "\aFFFFFFFFAuto teleport", true)
moriade.menu.consolefilter = ui.new_checkbox("AA", "Other", "\aFFFFFFFFConsole filter");
...
moriade.menu.clantag = ui.new_checkbox("AA", "Other", "\aFFFFFFFFClan tag");
moriade.menu.airstop = ui.new_checkbox("AA", "Fake lag", "\aFFFFFFFFAir stop")
moriade.menu.airstopkey = ui.new_hotkey("AA", "Fake lag", "\aFFFFFFFFFFAir stop", true);
...
for k, v in pairs(moriade.menu.builder_state) do
    moriade.menu.antiaim_elements[k] = {  
        ...
        antiaim_yaw_slider_speed = ui.new_slider("AA", "Anti-aimbot angles", "\aFFFFFFFF"..moriade.menu.builder_state[k].." Yaw Speed  ", 1, 40, 0, true, "t", 1, {[15] = "Slow"});
        ...
        antiaim_defensive_yaw_slider_speed = ui.new_slider("AA", "Anti-aimbot angles", "\aFFFFFFFF"..moriade.menu.builder_state[k].." Defensive Yaw Speed  ", 1, 40, 0, true, "t", 1, {[15] = "Slow"});
...
client.set_event_callback( "paint_ui", function(  )
    ...
    ui.set_visible(moriade.reference.other.fakelag_limit, anti_aim_tab)
    ui.set_visible(moriade.reference.other.fakelag_enable, anti_aim_tab)
    ui.set_visible(moriade.reference.other.fakelag_variance, anti_aim_tab)
    ui.set_visible(moriade.reference.other.fakelag_amount, anti_aim_tab)
    ui.set_visible(moriade.reference.other.slow_motion[2], anti_aim_tab)
    ui.set_visible(moriade.reference.other.slow_motion1, anti_aim_tab)
    ui.set_visible(moriade.reference.other.legs, anti_aim_tab)
    ui.set_visible(moriade.reference.other.hide_shots[2], anti_aim_tab)
    ui.set_visible(moriade.reference.other.hide_shots1, anti_aim_tab)
    ui.set_visible(moriade.reference.other.fake_peek[2], anti_aim_tab)
    ui.set_visible(moriade.reference.other.fake_peek1, anti_aim_tab)
end)
...
client.set_event_callback( "setup_command", function( arg )
    ...
    freestanding_bodyyaw = false
    moriade.anti_aim.yaw = "0"
...
local kill_say_text = {
        ...
        "ミ★ 𝗼𝘄𝗻𝗲𝗱 ★彡",
        "𝐰𝐫𝐚𝐢𝐭𝐡? 𝐚𝐧𝐠𝐞𝐥𝐰𝐢𝐧𝐠𝐬? 𝐡𝐲𝐬𝐭𝐞𝐫𝐢𝐚?! 𝐬𝐭𝐨𝐩 𝐮𝐬𝐞 𝐨𝐥𝐝 𝐥𝐮𝐚! 𝐠𝐞𝐭 𝐦𝐨𝐫𝐢𝐚𝐝𝐞";
        "·.★·.·´¯`·.·★ 1 ★·.·´¯`·.·★.·";
        "f12 XD, so ez for ᴍᴏʀɪᴀᴅᴇ ʀᴇꜱᴏʟᴠᴇʀ";
        "𝟙𝟛𝟛𝟟 𝕤𝕨𝕒𝕘";
        "ffffffffffffff";
        "-_- soooo ezzzzz";
    }
...
local info_panel = function()
    ...
    if ui.get(moriade.menu.watermark) == "Modern" then
    ...
    local text = string.format('moriade.lua | %s | %s | %s', steamname, math.floor(latency) .. 'ms', actual_time)
    local h, w = 20+dpi_scaling*3, renderer.measure_text(dpi.flags[2], text) + 10

    roundedRectangle(sx-w/2, sy-8, w, h, r,g,b,150,"", 4)
...
client.set_event_callback("pre_render", function()
    ...
    if contains(ui.get(moriade.menu.animbreak), "Kangaroo") then
    ...
    if contains(ui.get(moriade.menu.animbreak), "Static legs in air") then
        entity.set_prop(entity.get_local_player(), "m_flPoseParameter", 1, 6)
    end

    if contains(ui.get(moriade.menu.animbreak), "Pitch zero on land") then
        local on_ground = bit.band(entity.get_prop(entity.get_local_player(), "m_fFlags"), 1)
            ...
            if ground_ticks > ui.get(moriade.reference.other.fakelag_limit)+1 and end_time > globals.curtime() then
                entity.set_prop(entity.get_local_player(), "m_flPoseParameter", 0.5, 12)
            end
        end
    if contains(ui.get(moriade.menu.animbreak), "Earthquake") then
